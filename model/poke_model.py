...
def update_type(pokemon):
        with connection.cursor() as cursor:
            ...
            for t in poke["types"]:
                    ...
                    connection.commit() 
            return "Updated tpyes " + " ".join([t["type"]["name"] for t in poke["types"]])
...
def add(pokemon):
        with connection.cursor() as cursor:
            ...
            connection.commit() 
            update_type(pokemon)
            return "inserted pokemon '{}' :".format( pokemon['name'])      
...
def get_img(id):
    with connection.cursor() as cursor:
        query_select_img_pokemon = """ SELECT pokemon.sprites_back_shiny,pokemon.sprites_front_shiny
                                    FROM PokeCorp.pokemon  
                                    where pokemon.id =  '{}'""".format(id)
        cursor.execute(query_select_img_pokemon )
        pokemon_img = cursor.fetchone()
        if pokemon_img["sprites_back_shiny"] is None:
            update_img(id)
            return get_img(id)
        return pokemon_img["sprites_back_shiny"],pokemon_img["sprites_front_shiny"]
...
def update_img(id):
    ...
    with connection.cursor() as cursor:
        insert_img_pokemon_query = """UPDATE pokemon
                                       SET
                                        sprites_back_shiny = '{}',
                                        sprites_front_shiny = '{}'
                                       WHERE id = {};
                                    """.format(api_img_poke_back,api_img_poke,id)
        cursor.execute(insert_img_pokemon_query)
        connection.commit()
